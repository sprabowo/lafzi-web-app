...
import { Box, PseudoBox, Heading, Text, Button, useToast, Icon } from '@chakra-ui/core';
...
export default function Result({ ayats, status, info, onLoadMore }) {
  ...
  const handleClipboard = (text, label, copyright = ' \n\n (src: lafzi-web.vercel.app)') => {
    ...
    <>
      <Box mt="24px">
        {ayats.map((ayat) => (
          ...
          >
            <Box lang="ar" css={{ fontSize: '1.5rem', lineHeight: '2.12', textAlign: "right", fontFamily: "Utsmani" }} dir="rtl" dangerouslySetInnerHTML={{ __html: ayat.text_hilight }}></Box>
            <Box w="100%" mt="16px">
